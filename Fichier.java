...
public class Fichier
{
	...
	public Fichier() {				
			try {
				fR = new BufferedReader(new FileReader(new File("src/QCM.txt")));
			}
			catch (FileNotFoundException e1) {
				e1.printStackTrace();
			}			
			try {
				fW = new BufferedWriter(new FileWriter(new File("src/QCM.txt"),true));
			} catch (IOException e) {
				e.printStackTrace();
			}	
	}
	...
	public void modeAffiche(char mode) {
		assert(mode != 'X') : "Un objet de type Fichier n'a pas été ouvert correctement.";
	...
	public void ecrireInt(int tmp) throws IOException {
		...
		assert(chaine != null) : "Cette chaine est \"null\" !";
	...
	/*
	 * @pre -
	 * @post lit un fichier et renvoie une string si cela s'est bien passé ou Error IOException si cette erreur est apparue
	 */
	public String lire() {
		try {
